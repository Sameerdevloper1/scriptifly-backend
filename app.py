...
def extract_video_id(
    ...
    if host in {
        ...
        return (
            video_id
            if YOUTUBE_VIDEO_ID_PATTERN.fullmatch(
                video_id
            )
            else None
        )
    ...
    if (
        ...
        if (
                video_id and
                YOUTUBE_VIDEO_ID_PATTERN.fullmatch(
                    video_id
                )
        ):
            return video_id
        ...
        return jsonify(
            {
                ...
                "language": None,
                "transcript": None,
                "error": "The YouTube URL is required."
            }
        ), 400
        ...
        return jsonify(
            {
                ...
                "error": (
                    ...
                    else "Unable to retrieve the transcript."
                )
            }
        ), 422
...
if __name__ == "__main__":
    app.run(
        host="0.0.0.0",
        port=5001,  # Yeh 5001 hona chahiye!
        debug=False
    )
